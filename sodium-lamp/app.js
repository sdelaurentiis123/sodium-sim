...
const ids = [
  ...
  'departure-big','gas-temperature','excitation-temperature','optical-depth','d1-pop','d2-pop','ground-pop',
  ...
  'quench-heat','parasitic-light','boundary-heat','exhaust-sensible','storage-rate','energy-residual','fatal',
];
...
let volumeMode = 2, orbit = -.72, pitch = .18, paused = false, dragging = false, lastX = 0, lastY = 0;
...
function readControls({commitDesign = false} = {}) {
  ...
  const ends = [['dark','intense 589 nm'],['320 K',`${params[P.TMAX].toFixed(0)} K`],['10⁻⁹','10¹ / mode'],['no reaction','peak local rate']];
  ui['volume-definition'].textContent = descriptions[volumeMode];
  ui['volume-scale'].className = `volume-scale ${['sodium','thermal','photon','reaction'][volumeMode]}`;
...
fn flameHolder(r:f32,z:f32)->f32{let lipR=.5*p[21];let radial=exp(-pow((r-lipR)/max(.00055,.18*p[21]),2.));let axial=exp(-pow((z-(p[22]+.0018))/.0028,2.));return p[27]*radial*axial;}
...
fn qrate(T:f32,fuel:f32,ox:f32,water:f32)->f32{let h2=.15*fuel;let o2=p[8]*.95*ox;let h2o=.60*water;let n2=max(.1,1.-h2-o2-h2o);let s=h2+o2+h2o+n2;let keff=(h2*3.8e-16+o2*2.0e-16+h2o*5.0e-16+n2*2.0e-17)/s*p[13];return keff*p[6]/(KB*T);}
...
@fragment fn volume(in:O)->@location(0)vec4<f32>{let yaw=v[1];let pitch=v[2];let aspect=v[3];let mode=i32(v[6]);let ro=vec3(3.1*cos(yaw)*cos(pitch),3.1*sin(yaw)*cos(pitch),3.1*sin(pitch));let fw=normalize(-ro);let right=normalize(cross(fw,vec3(0.,0.,1.)));let up=cross(right,fw);let rd=normalize(fw+right*in.uv.x*aspect*.57+up*in.uv.y*.57);let hit=hitCylinder(ro,rd);if(hit.y<=max(hit.x,0.)){return vec4(.008,.01,.013,1.);}var col=vec3(0.);var alpha=0.;let start=max(hit.x,0.);let step=(hit.y-start)/56.;for(var i=0;i<56;i++){let t=start+(f32(i)+.5)*step;let pos=ro+rd*t;let angle=atan2(pos.y,pos.x);if(angle>-.45&&angle<1.05){continue;}let rr=length(pos.xy);let z=pos.z*.5+.5;let rM=rr*p[2];let zM=z*p[3];let q=cell(rr,z);let occ=photons(q)/max(modes(q.a.x),1.);let emit=clamp((q.b.x+q.b.y)*2e5+log2(1.+occ)*.04+q.d.w*.10,0.,1.);var value=emit;var rgb=sodiumMap(value);var density=.008+emit*.085;if(mode==1){value=clamp((q.a.x-p[18])/max(p[26]-p[18],1.),0.,1.);rgb=thermalMap(value);density=.005+value*.07;}if(mode==2){value=clamp((logten(max(occ,1e-9))+9.)/10.,0.,1.);let core=clamp(q.d.w*3.2,0.,1.);rgb=mix(photonMap(value),vec3(1.,.99,.88),core);density=.006+value*.11+core*.07;}if(mode==3){value=clamp(q.d.w*2.8,0.,1.);rgb=mix(vec3(.18,.30,1.),vec3(1.,.97,.58),smoothstep(.08,.75,value));density=.002+value*.17;}let fuelInner=.5*p[21];let fuelOuter=fuelInner+.00045;let shroudInner=.5*p[24];let shroudOuter=shroudInner+.00060;let belowLip=1.-smoothstep(p[22],p[22]+.00045,zM);let fuelTube=smoothstep(fuelInner-.00020,fuelInner,rM)*(1.-smoothstep(fuelOuter,fuelOuter+.00020,rM))*belowLip;let shroudTube=smoothstep(shroudInner-.00020,shroudInner,rM)*(1.-smoothstep(shroudOuter,shroudOuter+.00020,rM))*belowLip;let lipZ=smoothstep(p[22]-.00035,p[22],zM)*(1.-smoothstep(p[22],p[22]+.00035,zM));let fuelLip=(1.-smoothstep(fuelOuter,fuelOuter+.00020,rM))*lipZ;let shroudLip=smoothstep(fuelOuter,fuelOuter+.00020,rM)*(1.-smoothstep(shroudOuter,shroudOuter+.00020,rM))*lipZ;let nozzleMask=max(max(fuelTube,shroudTube),max(fuelLip,shroudLip));if(nozzleMask>.01){rgb=mix(rgb,vec3(.58,.66,.71),nozzleMask);density=max(density,.34*nozzleMask);}if(q.a.w>.5&&q.a.w<1.5){rgb=mix(vec3(.43,.52,.58),thermalMap(value),select(.12,.55,mode==1));density=select(.055,.085,mode==1);}else if(q.a.w>1.5){rgb=mix(rgb,vec3(.10,.25,.34),.22);}let da=density*(1.-alpha);col+=rgb*da*(1.18+hash(pos*90.+p[15])*.08);alpha+=da;let outerShell=smoothstep(.972,.995,rr);let wallDa=.018*outerShell*(1.-alpha);col+=vec3(.38,.78,.84)*wallDa;alpha+=wallDa;if(alpha>.97){break;}}return vec4(col,1.);}`;
...
async function main() {
  ...
  async function inspect(){
    ...
    let maxT=0,minT=Infinity,maxWallT=0,outerWallT=0,maxThroughWallDeltaK=0,maxInnerWallHeatFluxWm2=0,wallHotspotZM=0,cappedGasCells=0,gasCells=0,maxDeparture=0,maxNa=0,maxUpper=0,maxPhotons=0,maxReaction=0,flameBase=Infinity,flameTip=-Infinity,flameRadius=0,best=0,bestBand=-1,bandCell=0,bestFallback=-1,fallbackCell=0,totalEnergy=0,combustionW=0,atomicPumpW=0,quenchW=0,photonCount=0,excitedAtoms=0,boundaryHeatW=0,outflowSensibleW=0,escapePhotonsS=0,radialEscapePhotonsS=0;
    ...
    for(let j=0;j<NZ;j++)for(let i=0;i<NX;i++){
      ...
      } else {gasCells++;if(T>=params[P.TMAX]-.5)cappedGasCells++;}
      if(material===0&&z>=params[P.INSERTION]){maxReaction=Math.max(maxReaction,reaction);if(reaction>.03){flameBase=Math.min(flameBase,z);flameTip=Math.max(flameTip,z);flameRadius=Math.max(flameRadius,r);}}
    ...
    const spectrum=drawSpectrum({coreState,shellState,escapeByGroup,spontaneousByLine}),meltReferenceK=2323,thermochemistryLimited=cappedGasCells>0,wall=sapphireThermalAssessment({peakTemperatureK:maxWallT,outerSkinTemperatureK:outerWallT,maximumThroughWallDeltaK:maxThroughWallDeltaK,wallThicknessM:params[P.TW],maximumHeatFluxWM2:maxInnerWallHeatFluxWm2,meltReferenceK}),radialIncidentW=radialEscapePhotonsS*2.1035*CONSTANTS.eV/Math.max(1-boundary.reflectance,1e-9),lateralAreaM2=2*Math.PI*params[P.R]*params[P.L],lineExitanceWM2=radialIncidentW/Math.max(lateralAreaM2,1e-12),temperatureRatio=maxT/Math.max(operatingReference.stoichiometric.adiabatic_temperature_k,1);
