...
const ids = [
  ...
  'evidence-status','line-fuel-efficiency','pv-electric-current','generator-eff-current','required-light-large','required-light-laser','required-light-future','fatal','run-phase',
];
...
ui['operating-protocol'].addEventListener('change',()=>{
  if(ui['operating-protocol'].value==='july15'){
    ...
    ui.sodium.value=80;ui.oxygen.value=38;ui.pressure.value=1.4;
    params[P.SALT]=0;
  }
  readControls({commitDesign:true});
  initializeGPU?.();
...
const WGSL = /* wgsl */`
const PI:f32=3.14159265359;const C:f32=2.99792458e8;const KB:f32=1.380649e-23;const EV:f32=1.602176634e-19;const EN:f32=2.1035*EV;const MNA:f32=3.81754e-26;const MNA_AMU:f32=22.98976928;const KCHEM:f32=4e4;
...
// Form the reduced mass in atomic-mass units. Multiplying two kg-scale f32
// values first underflows to zero in WGSL and poisons the non-LTE solve.
fn meanRelSpeed(T:f32,partnerAMU:f32)->f32{let reducedAMU=MNA_AMU*partnerAMU/(MNA_AMU+partnerAMU);let mu=reducedAMU*1.6605390666e-27;return sqrt(8.*KB*T/(PI*mu));}
...
async function main() {
  ...
  async function inspect(){
    ...
    let invalidStateIndex=-1;for(let index=0;index<data.length;index++){if(!Number.isFinite(data[index])){invalidStateIndex=index;break;}}if(invalidStateIndex>=0){const cell=Math.floor(invalidStateIndex/FLOATS),field=invalidStateIndex%FLOATS;readback.unmap();reading=false;throw new Error(`Non-finite GPU state at cell ${cell}, field ${field}. The run was stopped instead of displaying corrupted physics.`);}
    ...
    if(ui['run-phase']){const tNow=params[P.TIME];ui['run-phase'].textContent=branchLost?'flame lost — the stabilized branch blew off; press Start / rerun or reduce flow':tNow<.008?'igniting — the bright full-chamber glow is the seeded starting condition, not solved combustion':tNow<.04?(stabilized?`settling — the seed is relaxing toward a small flame anchored at the burner lip · ${burningCells} reacting cells`:'settling — one-time ignition seed advecting downstream; watching for blowoff'):burningCells>0?`steady burn — anchored lip flame · peak gas ${maxGasT.toFixed(0)} K · the glow is dimmer than the seed flash because D-line emission falls steeply with temperature`:'no reaction — nothing is burning in the current state';ui['run-phase'].className=`run-phase ${branchLost?'danger':tNow<.04?'entering':''}`;}
  ...
  function tick(){const steps=paused||designPending?0:LIVE_STEPS_PER_FRAME;updateEngagementState();writeParams();const enc=device.createCommandEncoder();if(steps){const pass=enc.beginComputePass();for(let step=0;step<steps;step++){compute(pass,pipelines.advance);for(let i=0;i<LIVE_RADIATION_SWEEPS;i++)compute(pass,pipelines.radiation);params[P.TIME]+=params[P.DT];}pass.end();}const aspect=resize();device.queue.writeBuffer(viewBuffer,0,new Float32Array([0,orbit,pitch,aspect,0,0,volumeMode,0]));const bg=device.createBindGroup({layout:renderLayout,entries:[{binding:0,resource:{buffer:state[current]}},{binding:1,resource:{buffer:paramBuffer}},{binding:2,resource:{buffer:viewBuffer}}]}),rp=enc.beginRenderPass({colorAttachments:[{view:context.getCurrentTexture().createView(),clearValue:{r:.005,g:.006,b:.008,a:1},loadOp:'clear',storeOp:'store'}]});rp.setPipeline(renderPipeline);rp.setBindGroup(0,bg);rp.draw(3);rp.end();device.queue.submit([enc.finish()]);ui['sim-time'].value=`${params[P.TIME].toFixed(4)} s physical time`;if(frame++%60===0)inspect().catch(fail);requestAnimationFrame(tick);}
